...
exports.AppCommitWorkflow = void 0;
 ...
 */
async function pushWithBuiltInGitExtension(gitRoot) {
    const ext = vscode.extensions.getExtension("vscode.git");
    if (!ext) {
        throw new Error("VS Code Git extension is not available. Push from the Source Control view or install Git credentials (e.g. Git Credential Manager).");
    }
    if (!ext.isActive) {
        await ext.activate();
    }
    ...
    let repo = api.repositories.find((r) => (0, appReviewPaths_1.pathsEqualAbsolute)(r.rootUri.fsPath, norm));
    if (!repo && api.repositories.length === 1) {
        repo = api.repositories[0];
    }
    if (!repo) {
        throw new Error(`Git extension has no open repository for ${gitRoot}. Open that folder as the workspace root and try again.`);
    }
    await repo.push();
}
class AppCommitWorkflow {
    ...
    async commitAppReview(fileUri) {
        ...
        if (!this.gitStatusTracker.isDirtyForFile(abs)) {
            vscode.window.showInformationMessage("No uncommitted changes for this file (git status clean). Nothing to commit.");
            return;
        }
        ...
        try {
            ...
            const rel = mapping?.relativePath ?? (0, appReviewPaths_1.toPosixRepoPath)(path.relative(gitRoot, abs));
            ...
            if (wantMermaidIgnore) {
                ...
                if (!mermaidIgnorePreview.changed) {
                    vscode.window.showInformationMessage(".mermaidignore already lists this path. Only your diagram changes will be committed.");
                }
            }
            ...
            if (wantMermaidIgnore && mermaidIgnorePreview?.changed) {
                await fs_1.promises.writeFile(mermaidIgnorePreview.ignorePath, mermaidIgnorePreview.nextContent, "utf8");
            }
    ...
    async getCommitMessage(fileName) {
     ...
     */
    async promptMermaidIgnoreBeforeCommit(normalizedRels) {
        ...
        const choice = await vscode.window.showWarningMessage(message, { modal: true }, "Accept", "Reject");
        if (choice === undefined) {
            return null;
        }
     ...
     */
    async readMermaidIgnoreMergePreview(gitRoot, relPosixPaths) {
        const ignorePath = path.join(gitRoot, ".mermaidignore");
        let body = "";
        try {
            body = await fs_1.promises.readFile(ignorePath, "utf8");
        }
        catch {
            body = "";
        }
    ...
    async askAboutPush(cwd, absolutePathsToClearOnPush) {
        ...
        if (pushAction === "Push Now") {
            try {
                ...
                vscode.window.showErrorMessage(`Push failed: ${msg}`, "Open Source Control").then((choice) => {
...
exports.AppCommitWorkflow = AppCommitWorkflow;
//# sourceMappingURL=appCommitWorkflow.js.map