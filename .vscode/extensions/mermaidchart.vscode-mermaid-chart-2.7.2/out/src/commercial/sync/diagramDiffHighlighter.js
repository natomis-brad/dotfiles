...
exports.createHighlightInstructions = exports.calculateDiagramDiff = void 0;
 ...
 */
function edgeDomDataIdFromMapping(mapping) {
    if (!mapping) {
        return undefined;
    }
    const d = mapping.data;
    if (d && typeof d === 'object' && 'svgId' in d && d.svgId != null) {
        return String(d.svgId);
    }
 ...
 */
function initializeMermaidWithAST() {
 ...
 */
async function parseWithAST(code, diagramType) {
    try {
        initializeMermaidWithAST();
        const res = await mermaid_1.default.parse(code);
        if (!res?.diagram?.db) {
            return undefined;
        }
        // Extract AST using collab's approach
        const db = res.diagram.db;
        if (typeof db.getAST === 'function') {
            ...
            if (ast?.elementMappings) {
                ...
                if (ast.messages) {
                    console.log('[AST-Diff] Found messages property:', ast.messages);
                }
                if (ast.interactions) {
                    console.log('[AST-Diff] Found interactions property:', ast.interactions);
                }
            }
            return ast;
        }
 ...
 */
function deduplicateSequenceElementMappings(mappings) {
 ...
 */
async function calculateDiagramDiff(oldCode, newCode) {
...
exports.calculateDiagramDiff = calculateDiagramDiff;
/**
 * Calculate diff using AST element mappings (collab methodology)
 */
function calculateASTBasedDiff(oldAST, newAST, diagramType) {
    const diff = {
        addedNodes: [],
        modifiedNodes: [],
        removedNodes: [],
    ...
    function getLogicalId(element, diagramType) {
    ...
    function getBestVertex(elements, logicalId) {
        ...
        return candidates.reduce((best, current) => {
            ...
            if (currentData.type && !bestData.type)
                return current;
            if (bestData.type && !currentData.type)
                return best;
 ...
 */
async function createHighlightInstructions(diff) {
    const newDiagramInstructions = [];
    const oldDiagramInstructions = [];
    ...
    const findElementMapping = (ast, elementId) => {
     ...
     */
    const extractSvgSelector = (mapping) => {
        if (!mapping)
            return undefined;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const runtimeSelector = mapping.svgSelector;
     ...
     */
    const findParticipantSvgSelector = (ast, participantId) => {
        if (!ast?.elementMappings)
            return undefined;
    ...
    const createNodeInstruction = (elementId, ast, changeType) => {
    ...
    const createEdgeInstruction = (elementId, ast, changeType) => {
        const mapping = findElementMapping(ast, elementId);
...
exports.createHighlightInstructions = createHighlightInstructions;
/**
...
//# sourceMappingURL=diagramDiffHighlighter.js.map