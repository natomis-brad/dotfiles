...
exports.reviewChromeScript = exports.renderChangesList = exports.renderNowBeforeToggle = exports.renderMetaChips = exports.renderHeaderActionButtons = exports.renderThemeSelect = exports.renderSummaryChips = exports.reviewChromeStyles = exports.MERMAID_PREVIEW_THEMES = void 0;
/** Diagram themes available in the preview webview (matches ThemeSelector.svelte). */
exports.MERMAID_PREVIEW_THEMES = [
...
exports.reviewChromeStyles = reviewChromeStyles;
...
exports.renderSummaryChips = renderSummaryChips;
function filterPill(kind, label, count) {
    return (`<button type="button" class="mc-pill filter-${kind} mc-review-chrome" data-filter="${kind}" aria-pressed="false">` +
...
function renderThemeSelect(currentTheme) {
    const options = exports.MERMAID_PREVIEW_THEMES.map((t) => `<option value="${escapeHtml(t.key)}"${t.key === currentTheme ? " selected" : ""}>${escapeHtml(t.label)}</option>`).join("");
    return (`<label class="mc-pill mc-pill-theme mc-review-chrome" title="Diagram theme">` +
...
exports.renderThemeSelect = renderThemeSelect;
function renderHeaderActionButtons() {
    return (`<button type="button" class="mc-pill mc-pill-action mc-review-chrome" data-action="export-diagram" title="Save / export diagram">` +
...
exports.renderHeaderActionButtons = renderHeaderActionButtons;
function renderMetaChips(opts) {
    if (!opts.prRef?.trim()) {
        return "";
    }
    const ref = escapeHtml(opts.prRef.trim());
    return (`<span class="mc-pill mc-pill-branch mc-review-chrome" title="Pull request">` +
        `${BRANCH_ICON_SVG}` +
        `<span class="mc-pill-label">${ref}</span></span>`);
}
exports.renderMetaChips = renderMetaChips;
function renderNowBeforeToggle(activePhase = "now") {
...
exports.renderNowBeforeToggle = renderNowBeforeToggle;
const VISIBLE_PER_GROUP = 5;
const GROUP_ORDER = ["added", "modified", "removed"];
const GROUP_TITLES = {
    added: "Added",
    modified: "Changed",
    removed: "Removed",
...
exports.renderChangesList = renderChangesList;
function reviewChromeScript(_nonce) {
    return /* js */ `
    (function () {
      var w = window;
...
exports.reviewChromeScript = reviewChromeScript;
function changeRowHtml(c, hidden) {
    const hiddenClass = hidden ? " hidden" : "";
    return (`<div class="mc-change-row-wrap${hiddenClass}" data-kind="${c.kind}">` +
...
//# sourceMappingURL=reviewChrome.js.map