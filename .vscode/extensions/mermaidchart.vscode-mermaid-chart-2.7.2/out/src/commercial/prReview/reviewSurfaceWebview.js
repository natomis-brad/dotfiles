...
exports.REVIEW_COMMANDS = exports.openReviewSurfaceWebview = void 0;
const vscode = __importStar(require("vscode"));
const lineDiff_1 = require("./lineDiff");
const reviewChrome_1 = require("./reviewChrome");
const openReview_1 = require("./openReview");
function openReviewSurfaceWebview(info, fileName, oldContent, newContent, onAction, options = { isSignedIn: false }) {
    ...
    const panel = vscode.window.createWebviewPanel("mermaidPrReviewSurface", `${fileName} · Synced by Mermaid Sync`, { viewColumn: vscode.ViewColumn.One, preserveFocus: false }, { enableScripts: true, retainContextWhenHidden: true });
    const diff = (0, lineDiff_1.computeLineDiff)(oldContent, newContent);
    const nonce = makeNonce();
    panel.webview.html = renderHtml(panel.webview, nonce, info, fileName, diff.entries, diff.addedCount, diff.removedCount, options);
...
exports.openReviewSurfaceWebview = openReviewSurfaceWebview;
function renderHtml(webview, nonce, info, fileName, entries, addedCount, removedCount, options) {
    const csp = [
        "default-src 'none'",
...
<style>
  ...
  ${(0, reviewChrome_1.reviewChromeStyles)()}
  html, body {
    margin: 0;
    padding: 0;
    height: 100%;
...
function renderDiffChromeBlock(options) {
    if (!options.diffCounts || !options.changes?.length) {
        return "";
    }
    const summary = (0, reviewChrome_1.renderSummaryChips)(options.diffCounts);
    const changes = (0, reviewChrome_1.renderChangesList)(options.changes);
    const toggle = (0, reviewChrome_1.renderNowBeforeToggle)("now");
...
function reviewChromeHandlersScript() {
    return `
    (function () {
 ...
 */
function renderExtrasBlock(options) {
...
exports.REVIEW_COMMANDS = {
    accept: openReview_1.ACCEPT_COMMAND,
    reject: openReview_1.REJECT_COMMAND,
    edit: openReview_1.EDIT_COMMAND,
};
//# sourceMappingURL=reviewSurfaceWebview.js.map