...
const reviewChrome_1 = require("../commercial/prReview/reviewChrome");
function getWebviewHTML(panel, extensionPath, initialContent, currentTheme, validateOnly, options, highlightAddedNodeIds, prReview) {
    const scriptUri = panel.webview.asWebviewUri(vscode.Uri.file(path.join(extensionPath, "out", "svelte", "bundle.js")));
    const fontUrl = panel.webview.asWebviewUri(vscode.Uri.file(path.join(extensionPath, "media", "recursive-latin-full-normal.woff2")));
    panel.onDidDispose(() => {
    ...
    <body>
      ...
      <script nonce="${nonce}">
        ${prReview ? (0, reviewChrome_1.reviewChromeScript)(nonce) : ""}
      </script>
      <script>
        (function () {
...
function buildPrReviewHeader(ctx) {
    const summary = (0, reviewChrome_1.renderSummaryChips)(ctx.counts);
    const meta = (0, reviewChrome_1.renderMetaChips)({ prRef: ctx.prRef });
    const theme = (0, reviewChrome_1.renderThemeSelect)(ctx.currentTheme ?? "redux-dark");
    const actions = (0, reviewChrome_1.renderHeaderActionButtons)();
...
function buildPrReviewStage(ctx) {
    const changes = (0, reviewChrome_1.renderChangesList)(ctx.changes);
...
//# sourceMappingURL=previewTemplate.js.map