...
class RepairDiagram {
     ...
     */
    static async repairDiagram(originalCode, errorMessage, document) {
        ...
        try {
            await vscode.window.withProgress({
                ...
                if (response && response.result === 'ok' && response.code) {
                    ...
                    const choice = await vscode.window.showInformationMessage('✨ AI has repaired your diagram. What would you like to do?', { modal: false }, applyButton, viewDiffAndApplyButton);
     ...
     */
    static async showDiffView(originalCode, repairedCode, document) {
        try {
            ...
            // Open diff view
            await vscode.commands.executeCommand('vscode.diff', originalUri, repairedUri, 'AI Repair: Original ↔ Repaired', {
                preview: false,
     ...
     */
    static setupTempFileWatcher(tempFilePath, originalDocument, originalTempPath) {
        const originalUri = vscode.Uri.file(originalTempPath);
        ...
        const onTabCloseDisposable = vscode.window.tabGroups.onDidChangeTabs(({ closed }) => {
            for (const tab of closed) {
                if (tab.input instanceof vscode.TabInputTextDiff) {
                    ...
                    if (original.toString() === originalUri.toString() ||
     ...
     */
    static async updateOriginalDocument(newContent, originalDocument) {
        try {
            const fullRange = new vscode.Range(originalDocument.positionAt(0), originalDocument.positionAt(originalDocument.getText().length));
     ...
     */
    static extractMermaidCode(markdownText) {
     ...
     */
    static setupTempFileWatcherForTempDocument(repairedTempPath, originalDocument, originalUri, repairedUri) {
        let cleaned = false;
        const cleanup = async () => {
            if (cleaned) {
                return;
...
exports.RepairDiagram = RepairDiagram;
//# sourceMappingURL=repairDiagram.js.map