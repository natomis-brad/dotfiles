...
const repairDiagram_1 = require("./repairDiagram");
const themeConfig_1 = require("../../webview/src/themes/themeConfig");
...
class PreviewPanel {
    ...
    static setMcAPI(mcAPI) {
        PreviewPanel.mcAPI = mcAPI;
        repairDiagram_1.RepairDiagram.setMcAPI(mcAPI);
    }
    static createOrShow(document) {
    ...
    async update() {
        ...
        const vscodeThemeName = config.get("workbench.colorTheme", "Default Light+");
        console.log(`VS Code theme: ${vscodeThemeName} (${isDarkTheme ? "dark" : "light"})`);
        // Get the theme settings from configuration
        const darkTheme = config.get(DARK_THEME_KEY, "redux-dark");
        const lightTheme = config.get(LIGHT_THEME_KEY, "redux");
        const maxZoom = config.get(MAX_ZOOM, 5);
        const maxCharLength = config.get(MAX_CHAR_LENGTH, 90000);
        const maxEdges = config.get(MAX_EDGES, 1000);
        ...
        const vscodeThemeColors = (0, themeConfig_1.getThemeColors)(vscodeThemeName);
        console.log('Theme colors:', vscodeThemeColors);
        this.lastContent = this.document.getText() || " ";
        if (!this.panel.webview.html) {
            this.panel.webview.html = (0, previewTemplate_1.getWebviewHTML)(this.panel, extensionPath, this.lastContent, currentTheme, false, {
                maxZoom,
                maxCharLength,
                maxEdges,
            });
    ...
    setupListeners() {
        ...
        vscode.workspace.onDidChangeConfiguration((event) => {
            if (event.affectsConfiguration(DARK_THEME_KEY) ||
        ...
        this.panel.webview.onDidReceiveMessage(async (message) => {
            if (message.type === "error" && message.message) {
                ...
                await vscode.window.withProgress({
                    ...
                    await (0, renderService_1.saveDiagramAsPng)(this.document, message.pngBase64, this.lastContent);
                });
            }
                ...
                await vscode.window.withProgress({
                    ...
                    await (0, renderService_1.saveDiagramAsSvg)(this.document, message.svgBase64, this.lastContent);
                });
            }
    ...
    handleDiagramError(errorMessage) {
        const diagnostics = [];
        ...
        if (errorDetails) {
            ...
            const range = new vscode.Range(errorDetails.line, startCharacter, errorDetails.line, endCharacter);
            const diagnostic = new vscode.Diagnostic(range, `Syntax error: ${errorDetails.message}`, vscode.DiagnosticSeverity.Error);
    ...
    async handleRepairDiagram(code, errorMessage) {
        try {
            await repairDiagram_1.RepairDiagram.repairDiagram(code, errorMessage, this.document);
