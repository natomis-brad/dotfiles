...
class DiagramManager {
     ...
     */
    async renameDiagram(item) {
        if (!(item instanceof mermaidChartProvider_1.Document)) {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const projectId = (0, mermaidChartProvider_1.getProjectIdForDocument)(item.uuid);
                if (!projectId) {
                ...
                // Update local cache
                const cachedDiagram = (0, mermaidChartProvider_1.getDiagramFromCache)(item.uuid);
                if (cachedDiagram) {
    ...
    async deleteDiagram(item) {
        ...
        const confirmation = await vscode.window.showWarningMessage(`Are you sure you want to delete the diagram "${diagramTitle}"? This action cannot be undone.`, { modal: true }, 'Delete', 'Cancel');
    ...
    async duplicateDiagram(item) {
        ...
        try {
            ...
            if (error?.status === 402 || error?.response?.status === 402) {
                vscode.window.showErrorMessage('Unable to create duplicate diagram. You have reached the diagram limit for your free account. Please upgrade your Mermaid Chart subscription to create more diagrams.', 'Upgrade Subscription').then(selection => {
    ...
    async addDiagram(item, context) {
        ...
        if (!item.children) {
            projectId = (0, mermaidChartProvider_1.getProjectIdForDocument)(item.uuid);
            if (!projectId) {
        ...
        const selectedTemplate = await vscode.window.showQuickPick(templateEntries.map(([name, code]) => ({
            label: name,
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const initialContent = (0, frontmatter_1.ensureIdField)(selectedTemplate.code, newDocument.documentID);
                // Create temporary mermaid file with the document ID
                const editor = await (0, createFile_1.createMermaidFile)(context, initialContent, true);
                if (editor) {
                    // Refresh the tree view to show the new diagram
                    await this.provider.syncMermaidChart();
                    vscode.window.showInformationMessage(`Diagram "${finalName}" created successfully. Edit and save to sync with Mermaid Chart.`);
                }
                else {
            ...
            // Check if this is a 402 Payment Required error (free account limit exceeded)
            if (error?.status === 402 || error?.response?.status === 402) {
                vscode.window.showErrorMessage('Unable to create diagram. You have reached the diagram limit for your free account. Please upgrade your Mermaid Chart subscription to create more diagrams.', 'Upgrade Subscription').then(selection => {
                    if (selection === 'Upgrade Subscription') {
                        vscode.env.openExternal(vscode.Uri.parse('https://www.mermaidchart.com/pricing'));
                    }
                });
     ...
     */
    async linkDiagram(item) {
        if (!(item instanceof mermaidChartProvider_1.Document)) {
            vscode.window.showErrorMessage('Only diagrams can be linked.');
            return;
        }
        try {
            await vscode.commands.executeCommand("mermaidChart.insertUuidIntoEditor", item);
        }
        catch (error) {
    ...
    async viewDiagram(item) {
        ...
        try {
            ...
            if (error?.response?.status === 400 || error?.status === 400 ||
                ...
                vscode.window.showErrorMessage(`Cannot view diagram "${item.title}". The diagram may have syntax errors. Please check your diagram syntax on Mermaid Chart or edit it locally to fix any issues.`, 'Edit Locally', 'Edit in Mermaid Chart').then(selection => {
     ...
     */
    async editDiagramInMermaidChart(item) {
        if (!(item instanceof mermaidChartProvider_1.Document)) {
     ...
     */
    static registerCommands(context, mcAPI, provider) {
        const diagramManager = new DiagramManager(mcAPI, provider);
        // Register rename command
        const renameCommand = vscode.commands.registerCommand('mermaidChart.renameDiagram', async (item) => {
            await diagramManager.renameDiagram(item);
        });
        context.subscriptions.push(renameCommand);
        // Register duplicate command
        const duplicateCommand = vscode.commands.registerCommand('mermaidChart.duplicateDiagram', async (item) => {
            await diagramManager.duplicateDiagram(item);
        });
        context.subscriptions.push(duplicateCommand);
        // Register add diagram command
        const addDiagramCommand = vscode.commands.registerCommand('mermaidChart.addDiagram', async (item) => {
            await diagramManager.addDiagram(item, context);
        });
        context.subscriptions.push(addDiagramCommand);
        // Register delete command
        const deleteCommand = vscode.commands.registerCommand('mermaidChart.deleteDiagram', async (item) => {
            await diagramManager.deleteDiagram(item);
        });
        context.subscriptions.push(deleteCommand);
        // Register link diagram command
        const linkDiagramCommand = vscode.commands.registerCommand('mermaidChart.linkDiagram', async (item) => {
            await diagramManager.linkDiagram(item);
        });
        context.subscriptions.push(linkDiagramCommand);
        // Register view diagram command
        const viewDiagramCommand = vscode.commands.registerCommand('mermaidChart.viewDiagram', async (item) => {
        ...
        const editDiagramInMermaidChartCommand = vscode.commands.registerCommand('mermaidChart.editDiagramInMermaidChart', async (item) => {
        ...
        const editDiagramLocallyCommand = vscode.commands.registerCommand('mermaidChart.editDiagramLocally', async (item) => {
